 ...
 */
public class dataGenerator_ASOC
{
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException, IOException
	{
		...
		while (true)
		{
			...
			System.out.println("\t- 1- Generate test data for HRDF (New Asoc)");
			System.out.println("\t- 2- Generate Lists for HRDF (New Asoc)");
		...
		{
		case "1":
			generateDataAsocNew(150, 300); // generate test data for HRDF based on ASOC Job title index
			break;
		case "2":
			generateLists_HRDF();
			break;
	...
	public static void generateDataAsocNew(int jobCounts, int canCounts) throws SQLException, ClassNotFoundException
	{
		...
		jobCode = "isco_code";
		occupCode= "isco_code2";
		occupSRC = "taxonomies_hrdf.occupations_ar";
	...
	*/	System.out.println("\n\tStarting at: " + date);
	...
	public static void AssignNames_ar()
	{
		...
		try
		{
			...
			int nbre = 0;
			query = "SELECT candidate_id, language, gender FROM emp_hrdf.candidate";
			...
			while (rs.next())
			{
				...
				{
					stmt2.executeUpdate(
						"Update emp_hrdf.candidate set first_name=\""
							+ Names[nbre][0]
							+ "\", middle_name= \""
	...
	public static void generateEducation()
	{
		...
		try
		{
			...
			query = "select candidate_id, birth_date, timestampdiff(year,birth_date, curdate()) age FROM emp_hrdf.candidate where candidate_id not in (select distinct candidate_id from emp_hrdf.candidate_education)";
			...
			while (rs.next())
			{
				...
				for (int i = niv; i <= 10; i++)
				{
					...
					if (rs2.next())
					{
						query = "insert into emp_hrdf.candidate_education (candidate_id, education_name, education_level) values ("
							+ cId
							+ ", '"
	...
	public static void GenerateJobCompanies_ar()
	{
		...
		try
		{
			...
			stmt2 = conn.createStatement();
			ResultSet rs = null;
			String query = "SELECT count(*) from emp_hrdf.employer";
			rs = stmt.executeQuery(query);
			if (rs.next() && !rs.getString(1).equalsIgnoreCase("0"))
				return;

			//System.out.println (new Date());
			query = "SELECT * FROM taxonomies_hrdf.geoname where country ='SA' and feature_class='P' and population>10000";
			rs = stmt.executeQuery(query);
			int i = 0;
			while (rs.next())
			{
			...
			query = "SELECT * from taxonomies_hrdf.companies where country like '%SA%'";
			rs = stmt.executeQuery(query);
			Random rn = new Random();
			int nbre = 0;
			int j = 1;
			...
			while (rs.next())
			{
				...
				query = "insert into emp_hrdf.employer (employer_id, company_name, industry_sector, city, country, latitude, longitude) values ("
					+ j++
					+ ", \""
					+ rs.getString("name")
					+ "\", '"
			...
			//query = "update emp_hrdf.employer a set industry_sector=(select sector_id from asoc.sectors b where a.industry_sector=b.name_en) where a.industry_sector in (select name_en from asoc.sectors)";
			//stmt2.executeUpdate(query);
			System.out.println(new Date());
	...
	public static void assignJobCompany_ar()
	{
		...
		try
		{
			...
			String query = "update emp_hrdf.job a set job_sector = (select sector from taxonomies_hrdf.sector_ar b where left(a.job_occupation,6)=b.occup_code) where language='ar'";
			stmt2.executeUpdate(query);

			query = "SELECT distinct industry_sector FROM emp_hrdf.employer";
			rs = stmt.executeQuery(query);
			while (rs.next())
				EmpDic.put(rs.getString("industry_sector"), "");
			
			query = "SELECT employer_id, industry_sector FROM emp_hrdf.employer order by industry_sector, employer_id";
			rs = stmt.executeQuery(query);
			//String strValue;
			while (rs.next()) {
			...
			query = "SELECT count(*) from emp_hrdf.employer";
			rs = stmt.executeQuery(query);
			int counts = 0;
			if (rs.next())
			{
			...
			query = "SELECT job_id, job_title, job_sector from emp_hrdf.job";
			rs = stmt.executeQuery(query);
			rn = new Random();
			String strValue = "";
			while (rs.next())
			{
				...
				query = "update  emp_hrdf.job set employer_id='"
			...
			query = "update emp_hrdf.job a set job_sector = (select sector_ar from taxonomies_hrdf.sector_ar b where left(a.job_occupation,6)=b.occup_code) where language='ar'";
			stmt2.executeUpdate(query);
	...
	public static void generateJobLanguages() throws ClassNotFoundException
	{
		...
		try
		{
			...
			query = "SELECT job_id disc_title_en, proficiency_title_en from asoc.occup_languages, emp_hrdf.job where left(job_occupation,5)=left(occup_code,5)";
			rs = stmt.executeQuery(query);

			while (rs.next())
			{
				query = "insert into emp_hrdf.job_languages (job_code, language_name, language_level) values ('"
					+ rs.getString(1)
					+ "', '"
					+ rs.getString(2)
					+ "', '"
			...
			stmt2.executeUpdate("update emp_hrdf.job_languages set language_level='Beginner' where language_level='Basic'");
			stmt2.executeUpdate("update emp_hrdf.job_languages set language_level='Advanced' where language_level='Expert'");
			stmt.close();
			stmt2.close();
			conn.close();
	...
	public static void generateJobEducation_2() throws ClassNotFoundException
	{
		...
		try
		{
			...
			while (rs.next())
			{
				query = "insert into emp_hrdf.job_education (job_id, education_level, education_name) values ('"
					+ rs.getString(1)
					+ "', '"
					+ rs.getString(2)
					+ "', '"
			...
			{
				stmt2.executeUpdate("update emp_hrdf.job_education set education_level='" + edu[i][1] + "' where education_level='" + edu[i][0] + "'");
			}
			rs.close();

			stmt2.executeUpdate("update emp_hrdf.job_education set edu_discipline=(select distinct disc_title_en from asoc.discipline where occup_code=job_id limit 1)");
			stmt2.executeUpdate("update emp_hrdf.job_education set edu_field=(select distinct field_title_en from asoc.discipline where occup_code=job_id limit 1)");
			stmt.close();
			stmt2.close();
			conn.close();
	...
	public static void generateChildren()
	{
		...
		try
		{
			...
			query = "SELECT candidate_id, birth_date, timestampdiff(year,birth_date, curdate()) age, year(birth_date) Byear FROM emp_hrdf.candidate where marital_status!='Single' and candidate_id not in (select distinct candidate_id from emp_hrdf.candidate_children)";
			//System.out.println("\t- " + query);
			rs = stmt.executeQuery(query);
			Random rn = new Random();
			...
			while (rs.next())
			{
				...
				for (int i = 1; i <= nbre; i++)
				{
					...
					query = "insert into emp_hrdf.candidate_children (candidate_id, child_dob, child_gender) values ("
						+ cId
						+ ", '"
						+ Byear
						+ "-"
	...
	public static void generateZipCodes(Map<String, Integer> cities)
	{
		...
		try
		{
			...
			query = "SELECT candidate_id from emp_hrdf.candidate where candidate_id not in (select distinct candidate_id from emp_hrdf.candidate_locations)";
			//query = "SELECT candidate_id from emp_hrdf.candidate where zipcode is null and geo_latitude is null";
			//System.out.println(query);
			rs = stmt.executeQuery(query);
			Random rn = new Random();
			...
			for (; rs.next();)
			{
				...
				query = "insert into emp_hrdf.candidate_locations values ('"
					+ rs.getInt(1)
					+ "','"
					+ addresses[0]
					+ "', \""
	...
	public static void generateAddresses_sa(String addressType)
	{
		...
		try
		{
			...
			ResultSet rs = null;
			String[][] cities = new String[200][4];
			String query = "SELECT * FROM taxonomies_hrdf.geoname where feature_class='P' and population>10000";
			rs = stmt.executeQuery(query);
			int i = 0;
			while (rs.next())
			{
			...
			System.out.print("Generating Addresses ...\t");
			query = "SELECT candidate_id from emp_hrdf.candidate where candidate_id not in (select distinct candidate_id from emp_hrdf.candidate_locations where address_type='" + addressType + "')";
			//query = "SELECT candidate_id from emp_hrdf.candidate where zipcode is null and geo_latitude is null";
			//System.out.println(query);
			rs = stmt.executeQuery(query);
			for (; rs.next();)
			{
				...
				query = "insert into emp_hrdf.candidate_locations (candidate_id, city, country, geo_latitude, geo_longitude, address_type, distance) values ('"
					+ rs.getInt(1)
					+ "',\""
					+ cities[nbre][0]
					+ "\", '"
	...
	public static void generateZipCodes_old(Map<String, Integer> cities)
	{
		...
		try
		{
			...
			query = "SELECT candidate_id from emp_hrdf.candidate where zipcode is null";
			//query = "SELECT candidate_id from emp_hrdf.candidate where zipcode is null and geo_latitude is null";
			//System.out.println(query);
			rs = stmt.executeQuery(query);
			Random rn = new Random();
			...
			for (; rs.next();)
			{
				...
				query = "update emp_hrdf.candidate set zipcode ='"
					+ addresses[0]
					+ "', city='"
					+ addresses[1]
					+ "' where candidate_id="
	...
	public static void generateCandidates(int from, int to, int nbre, String genType) throws SQLException
	{
		...
		try
		{
			...
			rs = stmt.executeQuery("select max(candidate_id) from emp_hrdf.candidate");
			if (rs.next())
			{
				start = rs.getInt(1) + 1;
			}
			rs = stmt.executeQuery("select count(*) from emp_hrdf.employer");
			if (rs.next())
			{
			...
			for (i = start; i < nbre + start; i++)
			{
				...
				query = "insert into emp_hrdf.candidate (candidate_id,birth_date, gender, driving_licence, marital_status, candidate_Status, nationality, work_permit_validity, language) values ("
					+ i
					+ ",'"
					+ Bdate
					+ "', '"
				...
				//stmt.executeUpdate("update emp_hrdf.candidate set work_permit_validity='2099-12-31'  where nationality='SA'");
...
/*				String disability = pickValue(disabilities, rn.nextInt(totals[5]));
				...
				{
					query = "insert into emp_hrdf.candidate_disabilities (candidate_id, disability_name) values ("
						+ i
						+ ", \""
						+ disability
						+ "\");";
...
*/
				...
				query = "insert into emp_hrdf.candidate_ambitions (candidate_id,job_title, occupation, availability_date, salary, salary_period, "
				...
				for (j = y + age; j < year; j = j + jump)
				{
					...
					query = "insert into emp_hrdf.work_experience (candidate_id, job_title_we, isco_code, full_part_time, start_we, end_we, employer_id, language) values ("
						+ i
						+ ", \""
						+ jobTitle
						+ "\", \""
					...
					query = "SELECT ConceptURI, ConceptPT FROM asoc.skills where ConceptURI in (select hasRelatedConcept from escoskos.occupation_skill where isRelatedConcept='"
						+ ocId
						+ "')";
					//System.out.println(query);
					rs = stmt.executeQuery(query);
					...
					for (; rs.next();)
					{
						...
						//query = "insert into emp_hrdf.candidate_skills (job_id, skill_id) values (" + i + ", '" +  skillId + "');";
						//System.out.println("\t" + query);
						//stmt.executeUpdate(query);
					}
					rs.close();
				...
				while (it.hasNext())
				{
					...
					query = "insert into emp_hrdf.candidate_skills (candidate_id, skill_id, skill_name) values ("
						+ i
						+ ", '"
						+ entry.getKey()
						+ "', \""
	...
	public static void GenCandidateSkills()
	{
		...
		try
		{
			...
			String query = "SELECT candidate_id, job_title from emp_hrdf.candidate_ambitions where candidate_id not in (select distinct candidate_id from emp_hrdf.candidate_skills)";
			//System.out.println(query);
	...
	public static void generateCandidateLanguages()
	{
		...
		try
		{
			...
			rs = stmt.executeQuery(
				"SELECT Candidate_id, nationality FROM emp_hrdf.Candidate where candidate_id not in (select distinct candidate_id from emp_hrdf.candidate_languages)");
			...
			while (rs.next())
			{
				...
				{
					Map.Entry<String, String> entry = it.next();
					query = "insert into emp_hrdf.Candidate_Languages (candidate_id, language_id, level) values ("
						+ cId
						+ ", '"
						+ entry.getKey()
						+ "', '"
	...
	public static void loadJobs(int trg_occup, String occupSRC)
	{
		...
		try
		{
			...
			{
				query = "SELECT distinct job_code, job_occupation from emp_hrdf.job where job_id>=" + from_ + " and job_id<=" + to_;
				query2 = "SELECT distinct " + jobCode + ", job_title_en, job_title_ar FROM " + occupSRC 
					+ " where job_code in (SELECT distinct job_code from emp_hrdf.job where job_id>=" + from_ + " and job_id<=" + to_ + ")";
			}
	...
	*/
	public static void generateJobs(int trg_occup, int counts, String tableName)
	{
		...
		try
		{
			...
			rs = stmt.executeQuery("select max(job_id) from emp_hrdf.job");
			int start = 1;
			if (rs.next())
			{
				start = rs.getInt(1) + 1;
...
*/
			...
			while (j <= counts && index > 0)
			{
				...
				query = "insert into emp_hrdf.job (job_id, job_code, job_title, full_part_time, job_schedule, job_occupation, salary, salary_period, "
				...
				stmtS.executeUpdate(query);
				/*
				query = "SELECT ConceptURI, ConceptPT FROM asoc.skills where ConceptURI in (select hasRelatedConcept from escoskos.occupation_skill where isRelatedConcept='" + ocId + "')" + order ;
				System.out.println(query);
				...
				while(rsS.next() && j++<(r+2)){
				    ...
				    query = "insert into emp_hrdf.job_skills (job_id, skill_id) values (" + i + ", '" +skillId + "');";
				    System.out.println("\t" + query);
	 ...
	 */
	public static void generateLists_HRDF()
	{
		try
		{
			...
			System.out.println(new Date());
			String filepath = "c:\\ELISE6.6\\dictionaries\\HRDFDictionary1.1\\list\\ct_maji.ed";
			PrintWriter writer = new PrintWriter(filepath);
			filepath = "c:\\ELISE6.6\\dictionaries\\HRDFDictionary1.1\\list\\ct_majii.ed";
...
/*			filepath = "c:\\ELISE6.6\\dictionaries\\HRDFDictionary1.1\\list\\ct_occu.ed";
			...
			writer.close();
*/
			filepath = "c:\\ELISE6.6\\dictionaries\\HRDFDictionary1.1\\list\\ct_sec.ed";
			System.out.println(new Date());
			writer = new PrintWriter(filepath);
			// query to retrieve occupations
			...
			filepath = "c:\\ELISE6.6\\dictionaries\\HRDFDictionary1.1\\list\\ct_ind.ed";
			System.out.println(new Date());
			writer = new PrintWriter(filepath);
			// query to retrieve occupations
	...
	{ //synonyms and similars from Janzz
		...
		try
		{
			...
			stmt4.executeUpdate("delete from emp_hrdf.candidate_education");

			rs = stmt.executeQuery(
				"SELECT candidate_id, job_title, left(occupation,6) occupation, sector, 'en' language FROM emp_hrdf.candidate_ambitions");
			...
			while (rs.next())
			{
				...
				update = "insert into emp_hrdf.candidate_education (candidate_id, education_level, education_area, education_field) values (\"" + rs.getString(1) + "\",\"";
			...
			stmt4.executeUpdate("update emp_hrdf.candidate_education set education_area=null where education_area=''");
			stmt4.executeUpdate("update emp_hrdf.candidate_education set education_field=null where education_field=''");
	...
	{ //synonyms and similars from Janzz
		...
		try
		{
			...
			stmt4.executeUpdate("delete from emp_hrdf.job_education");

			rs = stmt.executeQuery(
				"SELECT job_id, job_title, left(job_occupation,6) occupation, job_sector, 'en' language FROM emp_hrdf.job");
			...
			{
				//System.out.println(rs.getString(1) + ": " + rs.getString(2));
				update = "insert into emp_hrdf.job_education (job_id, education_level, education_area, education_field) values (\"" + rs.getString(1) + "\",\"";
				query = "SELECT oe.job_code, e.edu_code, e.edu_title_" + rs.getString("language") + " FROM asoc.occupation_education oe, asoc.education e where oe.edu_code=e.edu_code"
					+ " and oe.job_code=\"" + rs.getString(3) + "\"";
				//System.out.println(query);
				rs2 = stmt2.executeQuery(query);
			...
			stmt4.executeUpdate("update emp_hrdf.job_education set education_area=null where education_area=''");
			stmt4.executeUpdate("update emp_hrdf.job_education set education_field=null where education_field=''");
	...
	{
		String[] queries = {
			"delete FROM emp_hrdf.job_education",
			"delete FROM emp_hrdf.job_languages",
			"delete FROM emp_hrdf.work_experience",
			"delete FROM emp_hrdf.job",
			"delete FROM emp_hrdf.employer",
			"delete FROM emp_hrdf.job_skills",
			"delete FROM emp_hrdf.candidate_education",
			"delete FROM emp_hrdf.candidate_certificates",
			"Delete from emp_hrdf.candidate_skills",
			"Delete from emp_hrdf.candidate_languages",
			"Delete from emp_hrdf.candidate_related",
			"Delete from emp_hrdf.candidate_education",
			"Delete from emp_hrdf.candidate_children",
			"Delete from emp_hrdf.candidate_disabilities",
			"Delete from emp_hrdf.candidate_ambitions",
			"Delete from emp_hrdf.candidate_locations",
			"Delete from emp_hrdf.candidate_benefits",
			"Delete from emp_hrdf.candidate_disabilities",
			"delete FROM emp_hrdf.candidate",
			};
	...
	public static void fullStatistics() throws ClassNotFoundException, SQLException
	{
		...
		rs = stmt.executeQuery("select count(*) FROM emp_hrdf.job");
		for (;rs.next();)
			System.out.println("Total number of jobs: " + rs.getString(1)); 
		
		System.out.println(" - Languages"); 
		rs = stmt.executeQuery("SELECT  language, COUNT(language) AS `Count`, concat(FORMAT(((COUNT(language) * 100) / newJobs.iCount),2),'%') AS `Percentage` FROM   emp_hrdf.job, (SELECT COUNT(language) AS iCount FROM emp_hrdf.job) newJobs GROUP BY language");
		for (;rs.next();) 
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getInt(2) + " jobs - " + rs.getString(3)); 
		
		System.out.println(" - telework"); 
		rs = stmt.executeQuery("SELECT  telework, COUNT(telework), concat(FORMAT(((COUNT(telework) * 100) / newJobs.iCount),2),'%') FROM   emp_hrdf.job, (SELECT COUNT(telework) AS iCount FROM emp_hrdf.job) newJobs GROUP BY telework");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		
		System.out.println(" - full_part_time"); 
		rs = stmt.executeQuery("SELECT  full_part_time, COUNT(full_part_time), concat(FORMAT(((COUNT(full_part_time) * 100) / newJobs.iCount),2),'%') FROM   emp_hrdf.job, (SELECT COUNT(full_part_time) AS iCount FROM emp_hrdf.job) newJobs GROUP BY full_part_time");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		
		System.out.println(" - job_shift_type"); 
		rs = stmt.executeQuery("SELECT  job_shift_type, COUNT(job_shift_type), concat(FORMAT(((COUNT(job_shift_type) * 100) / newJobs.iCount),2),'%') FROM   emp_hrdf.job, (SELECT COUNT(job_shift_type) AS iCount FROM emp_hrdf.job) newJobs GROUP BY job_shift_type");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		...
		rs = stmt.executeQuery("SELECT  job_contract_type, COUNT(job_contract_type), concat(FORMAT(((COUNT(job_contract_type) * 100) / newJobs.iCount),2),'%') FROM   emp_hrdf.job, (SELECT COUNT(job_contract_type) AS iCount FROM emp_hrdf.job) newJobs GROUP BY job_contract_type");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		
		System.out.println(" - travel_to_work"); 
		rs = stmt.executeQuery("SELECT  travel_to_work, COUNT(travel_to_work), concat(FORMAT(((COUNT(travel_to_work) * 100) / newJobs.iCount),2),'%') FROM   emp_hrdf.job, (SELECT COUNT(travel_to_work) AS iCount FROM emp_hrdf.job) newJobs GROUP BY travel_to_work");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		
		System.out.println(" - desired_gender"); 
		rs = stmt.executeQuery("SELECT  desired_gender, COUNT(desired_gender), concat(FORMAT(((COUNT(desired_gender) * 100) / newJobs.iCount),2),'%') FROM   emp_hrdf.job, (SELECT COUNT(desired_gender) AS iCount FROM emp_hrdf.job) newJobs GROUP BY desired_gender");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		
		/************** Job Seeker ***************************/
		System.out.println("\n\n *** Job Seeker *** "); 
		rs = stmt.executeQuery("select count(*) FROM emp_hrdf.candidate");
		for (;rs.next();)
			System.out.println("Total number of jobseekers: " + rs.getString(1)); 
		
		System.out.println(" - Languages"); 
		rs = stmt.executeQuery("SELECT  language, COUNT(language) AS `Count`, concat(FORMAT(((COUNT(language) * 100) / newJobs.iCount),2),'%') AS `Percentage` FROM   emp_hrdf.candidate, (SELECT COUNT(language) AS iCount FROM emp_hrdf.candidate) newJobs GROUP BY language");
		for (;rs.next();) 
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getInt(2) + " candidates - " + rs.getString(3)); 
		
		System.out.println(" - driving_licence"); 
		rs = stmt.executeQuery("SELECT  driving_licence, COUNT(driving_licence), concat(FORMAT(((COUNT(driving_licence) * 100) / newJobs.iCount),2),'%') FROM   emp_hrdf.candidate, (SELECT COUNT(driving_licence) AS iCount FROM emp_hrdf.candidate) newJobs GROUP BY driving_licence");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 
		
		System.out.println(" - marital_status"); 
		rs = stmt.executeQuery("SELECT  marital_status, COUNT(marital_status), concat(FORMAT(((COUNT(marital_status) * 100) / newJobs.iCount),2),'%') FROM   emp_hrdf.candidate, (SELECT COUNT(marital_status) AS iCount FROM emp_hrdf.candidate) newJobs GROUP BY marital_status");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 
		
		System.out.println(" - nationality"); 
		rs = stmt.executeQuery("SELECT  nationality, COUNT(nationality), concat(FORMAT(((COUNT(nationality) * 100) / newJobs.iCount),2),'%') FROM   emp_hrdf.candidate, (SELECT COUNT(nationality) AS iCount FROM emp_hrdf.candidate) newJobs GROUP BY nationality");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 
		
		System.out.println(" - gender"); 
		rs = stmt.executeQuery("SELECT  gender, COUNT(gender), concat(FORMAT(((COUNT(gender) * 100) / newJobs.iCount),2),'%') FROM   emp_hrdf.candidate, (SELECT COUNT(gender) AS iCount FROM emp_hrdf.candidate) newJobs GROUP BY gender");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 
	...
	{
		String[] queries = {
			"delete FROM emp_hrdf.job where job_id>=" + from + " and job_id<=" + to,
			"delete FROM emp_hrdf.work_experience where candidate_id>=" + from + " and candidate_id<=" + to,
/*			"delete FROM emp_hrdf.employer where employer_id not in (select employer_id from emp_hrdf.job where job_id>="
				+ from
				+ " and job_id<="
				+ to
				+ ")",
*/			"delete FROM emp_hrdf.job_skills where job_id>=" + from + " and job_id<=" + to,
			"delete FROM emp_hrdf.candidate_certificates where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_hrdf.candidate_skills where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_hrdf.candidate_languages where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_hrdf.candidate_related where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_hrdf.candidate_education where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_hrdf.candidate_children where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_hrdf.candidate_disabilities where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_hrdf.candidate_ambitions where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_hrdf.candidate_locations where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_hrdf.candidate_benefits where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_hrdf.candidate_disabilities where candidate_id>=" + from + " and candidate_id<=" + to,
			"delete FROM emp_hrdf.candidate where candidate_id>=" + from + " and candidate_id<=" + to };
	...
	{
		String[] queries = {
			"delete FROM emp_hrdf.job where job_id>=" + from,
			"delete FROM emp_hrdf.work_experience where candidate_id>=" + from,
			//"delete FROM emp_hrdf.employer",
			"delete FROM emp_hrdf.job_skills where job_id>=" + from,
			"delete FROM emp_hrdf.candidate_certificates where candidate_id>=" + from,
			"Delete from emp_hrdf.candidate_skills where candidate_id>=" + from,
			"Delete from emp_hrdf.candidate_languages where candidate_id>=" + from,
			"Delete from emp_hrdf.candidate_related where candidate_id>=" + from,
			"Delete from emp_hrdf.candidate_education where candidate_id>=" + from,
			"Delete from emp_hrdf.candidate_children where candidate_id>=" + from,
			"Delete from emp_hrdf.candidate_disabilities where candidate_id>=" + from,
			"Delete from emp_hrdf.candidate_ambitions where candidate_id>=" + from,
			"Delete from emp_hrdf.candidate_locations where candidate_id>=" + from,
			"Delete from emp_hrdf.candidate_benefits where candidate_id>=" + from,
			"Delete from emp_hrdf.candidate_disabilities where candidate_id>=" + from,
			"delete FROM emp_hrdf.candidate where candidate_id>=" + from };
