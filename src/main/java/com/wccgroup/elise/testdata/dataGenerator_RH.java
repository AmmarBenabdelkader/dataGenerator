...
public class dataGenerator_RH
{
	...
	{
		if (args.length!=3) {
			System.out.println("wrong arguments:\n\tUsage: java -jar dataGenerator_HR.jar NbreOfObjs steps start");
		...
		try
		{
			...
			String query = "SELECT * FROM emp_rh.transactiontable where (status!=3 and status!=9)";
			 ...
			 */			boolean indicator = true;
			 for (int s=1; s<=steps; s++) {
				 ...
				 assignNames(conn,jsCounts/steps);
		...
		//fullStatistics(conn); // perform statistics on the data
	 ...
	 */
	public static void executeFinalUpdates(Connection conn)
	{
	...
	public static void assignNames(Connection conn, int maxinput)
	{
		...
		try
		{
			...
			String query = "";
			query = "SELECT * from taxonomies_rh.name_us";
	...
	public static void createCandidateContacts(Connection conn)
	{
		...
		try
		{
			...
			squery.append("insert into emp_rh.candidate_contacts (first_name, last_name, gender, candidate_id) values ");

			String query = "";
			query = "SELECT * from taxonomies_rh.name_us order by firstname";
			ResultSet rs = stmt.executeQuery(query);
			int i = 0;
			while (rs.next())
			{
			...
			Random rn = new Random();
			query = "SELECT candidate_id FROM emp_rh.candidate where candidate_id not in (select candidate_id from emp_rh.candidate_contacts)";
	...
	public static void generateEmployers(String country, int population, Connection conn)
	{
		...
		try
		{
			...
			String query = "SELECT count(*) from emp_rh.employer";
			ResultSet rs = stmt.executeQuery(query);
			if (rs.next() && !rs.getString(1).equalsIgnoreCase("0"))
				return;

			query = "SELECT * FROM taxonomies_rh.geodata where country='" + country + "' and feature_class='P' and population>" + population;
			...
			StringBuilder squery = new StringBuilder();
			squery.append("insert into emp_rh.employer (employer_id, company_name, industry_sector, city, country, latitude, longitude) values ");
			query = "SELECT * from taxonomies_rh.companies where country='" + country + "'";
			rs = stmt.executeQuery(query);
			Random rn = new Random();
			int nbre = 0;
			int j = 1;
	...
	public static void generateAddresses(Connection conn)
	{
		...
		try
		{
			...
			String query = "SELECT * FROM taxonomies_rh.geodata where country='" + country + "' and feature_class='P' and population>" + population;
			//System.out.println(query);
			ResultSet rs = stmt.executeQuery(query);
			int i = 0;
			while (rs.next())
			...
			for (int k=0; k<addressType.length; k++) {
				...
				StringBuilder squery = new StringBuilder();
				squery.append("insert into emp_rh.candidate_locations (candidate_id, city, country, geo_latitude, geo_longitude, address_type, distance) values ");

				query = "SELECT candidate_id from emp_rh.candidate where candidate_id not in (select distinct candidate_id from emp_rh.candidate_locations where address_type='" + addressType[k] + "')";
				rs = stmt.executeQuery(query);
				int j=0;
				for (; rs.next();)
				{
	...
	public static int generateCandidates(int from, int to, int nbre, int start, Connection conn) throws SQLException
	{
		...
		try
		{
			...
			ResultSet rs = stmt.executeQuery("select count(*) from emp_rh.employer");
			if (rs.next())
			{
			...
			//System.out.print("\t- ");
			squery1.append("insert into emp_rh.candidate (candidate_id,birth_date, driving_licence, marital_status, candidate_Status, nationality, "
				+ "work_permit_validity, language,first_name,last_name,email, gender) values ");
			squery2.append("insert into emp_rh.candidate_ambitions (candidate_id,job_code, occupation, availability_date, salary, salary_period, "
			...
			squery3.append("insert into emp_rh.work_experience (candidate_id, job_title_we, isco_code, full_part_time, start_we, end_we, employer_id, employee_id, language) values ");
			for (i = start; i < nbre + start; i++)
			{
				...
				query = "SELECT ConceptPT, ConceptPT_en FROM taxonomies_rh.occupation where Parent_ISCOcode='"
					+ occupation.substring(occupation.indexOf("##") + 2) + "'";
	...
	public static void GenCandidateSkills_(Connection conn)
	{
		Date date = new Date();
		try
		{
			...
			String query = "SELECT candidate_id, job_code, occupation from emp_rh.candidate_ambitions where candidate_id not in (select distinct candidate_id from emp_rh.candidate_skills)";
			//System.out.println(query);
			...
			while (rsC.next())
			{
				...
				query = "SELECT SUBSTRING_INDEX(hasRelatedConcept, '/', -1) skill_id, ConceptPT skill_name FROM taxonomies_rh.occupation_skill,taxonomies_rh.skills "
					+ "where hasRelatedConcept=conceptURI and SUBSTRING_INDEX(isRelatedConcept, '/', -1)= '" + rsC.getString(2) + "'";
				//System.out.println(query);
				...
				while (it.hasNext() && c < i)
				{
					...
					query = "insert into emp_rh.candidate_skills (candidate_id, skill_id, skill_name, skill_type) values ("
	...
	public static void GenCandidateSkills(Connection conn)
	{
		...
		try
		{
			...
			StringBuilder squery = new StringBuilder();
			squery.append("insert into emp_rh.candidate_skills (candidate_id, skill_id, skill_name, skill_type) values ");

			String query = "SELECT SUBSTRING_INDEX(isRelatedConcept, '/', -1) occupation, SUBSTRING_INDEX(hasRelatedConcept, '/', -1) skill_id, skill_name "
				+ "FROM taxonomies_rh.occupation_skill order by occupation"; 
			ResultSet rs = stmt.executeQuery(query);
			...
			while (rs.next())
			{
				...
				query = "SELECT candidate_id, job_code, occupation from emp_rh.candidate_ambitions where job_code='" + tempOccup 
					+ "' and candidate_id not in (select distinct candidate_id from emp_rh.candidate_skills)";
				//System.out.println(query);
	...
	public static void GenCandidateCertificates(Connection conn)
	{
		...
		try
		{
			...
			String query = "SELECT candidate_id, job_code, occupation from emp_rh.candidate_ambitions where candidate_id not in (select distinct candidate_id from emp_rh.candidate_certificates)";
			StringBuilder squery = new StringBuilder();
			squery.append("insert into emp_rh.candidate_certificates (candidate_id, certificate_id, certificate_type, validity_date) values ");
			//System.out.println(query);
			ResultSet rsC = stmtC.executeQuery(query);
			Map<String, String> skills = new HashMap<String, String>();
			while (rsC.next())
			{
				...
				query = "SELECT SUBSTRING_INDEX(hasRelatedConcept, '/', -1) certificatel_id FROM taxonomies_rh.occupation_qualification "
	...
	public static void generateCandidateLanguages(Connection conn)
	{
		...
		StringBuilder squery = new StringBuilder();
		squery.append("insert into emp_rh.candidate_languages (candidate_id, language_id, level) values ");
		...
		try
		{
			...
			ResultSet rs = stmt.executeQuery(
				"SELECT Candidate_id, nationality FROM emp_rh.candidate where candidate_id not in (select distinct candidate_id from emp_rh.candidate_languages)");
	 ...
	 * @return void
	 */
	public static void loadJobs(String occupSRC, Connection conn)
	{
		try
		{
			Statement stmt = conn.createStatement();

			//String query = "SELECT SUBSTRING_INDEX(conceptURI, '/', -1) jobCode, Parent_ISCOcode occupCode from escoskos.occupation_es where Parent_ISCOcode in (select left(occup_code,4) from asoc3.occupation where occup_title_es is not null)";
			//String query2= "SELECT SUBSTRING_INDEX(conceptURI, '/', -1) jobCode, ConceptPT_en job_title_en, ConceptPT  job_title_es from escoskos.occupation_es where Parent_ISCOcode in (select left(occup_code,4) from asoc3.occupation where occup_title_es is not null)";
			String query = "SELECT SUBSTRING_INDEX(conceptURI, '/', -1) jobCode, Parent_ISCOcode occupCode from " + occupSRC + " where Parent_ISCOcode !=''";
			String query2= "SELECT SUBSTRING_INDEX(conceptURI, '/', -1) jobCode, ConceptPT_en job_title_en from " + occupSRC + " where Parent_ISCOcode  !=''";
			//System.out.println("\n" + query);
			ResultSet rs = stmt.executeQuery(query);
	...
	{ //synonyms and similars from Janzz
		...
		try
		{
			...
			StringBuilder squery = new StringBuilder();
			squery.append("insert into emp_rh.candidate_education (candidate_id, education_level, education_name, education_area, education_field) values ");

			System.out.print("\t\t+ Education \t\t--> " );

			//stmt3.executeUpdate("delete from emp_rh.candidate_education");

			ResultSet rs = stmt.executeQuery(
				"SELECT candidate_id, job_title, occupation, sector, 'en' language FROM emp_rh.candidate_ambitions where candidate_id not in (select candidate_id from emp_rh.candidate_education)");
			...
			while (rs.next())
			{
				...
				query = "SELECT oe.occup_code, e.code, e.title_" + rs.getString("language") + " FROM taxonomies_rh.occupationeducation_asoc oe, taxonomies_rh.education_asoc e where oe.edu_code=e.code"
	...
	public static void fullClean(Connection conn) throws ClassNotFoundException, SQLException
	{
		Date d = new Date();
		String[] queries = {
			"delete FROM emp_rh.work_experience",
			/*			"delete FROM emp_rh.employer",
			"delete FROM emp_rh.job_education where candidate_id not in (select candidate_id from emp_rh.transactiontable)",
			"delete FROM emp_rh.job_certificates where candidate_id not in (select candidate_id from emp_rh.transactiontable)",
			"delete FROM emp_rh.job_skills where candidate_id not in (select candidate_id from emp_rh.transactiontable)",
			"delete FROM emp_rh.job where candidate_id not in (select candidate_id from emp_rh.transactiontable)",
			"Delete from emp_rh.candidate_related where candidate_id not in (select candidate_id from emp_rh.transactiontable)",
			"Delete from emp_rh.candidate_children where candidate_id not in (select candidate_id from emp_rh.transactiontable)",
			"Delete from emp_rh.candidate_disabilities where candidate_id not in (select candidate_id from emp_rh.transactiontable)",
			 */			
			"delete FROM emp_rh.candidate_contacts",
			 "delete FROM emp_rh.candidate_education",
			 "delete FROM emp_rh.candidate_certificates",
			 "Delete from emp_rh.candidate_skills",
			 "Delete from emp_rh.candidate_languages",
			 "Delete from emp_rh.candidate_education",
			 "Delete from emp_rh.candidate_ambitions",
			 "Delete from emp_rh.candidate_locations",
			 "delete FROM emp_rh.candidate",
			 "delete FROM emp_rh.branch",
		};
	 ...
	 */
	public static void fullStatistics(Connection conn) throws ClassNotFoundException, SQLException
	{
		Statement stmt = conn.createStatement();
		ResultSet rs = null;

		/*		System.out.println(" - telework"); 
		rs = stmt.executeQuery("SELECT  telework, COUNT(telework), concat(FORMAT(((COUNT(telework) * 100) / newJobs.iCount),2),'%') FROM   emp_rh.job, (SELECT COUNT(telework) AS iCount FROM emp_rh.job) newJobs GROUP BY telework");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 

		System.out.println(" - full_part_time"); 
		rs = stmt.executeQuery("SELECT  full_part_time, COUNT(full_part_time), concat(FORMAT(((COUNT(full_part_time) * 100) / newJobs.iCount),2),'%') FROM   emp_rh.job, (SELECT COUNT(full_part_time) AS iCount FROM emp_rh.job) newJobs GROUP BY full_part_time");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 

		System.out.println(" - job_shift_type"); 
		rs = stmt.executeQuery("SELECT  job_shift_type, COUNT(job_shift_type), concat(FORMAT(((COUNT(job_shift_type) * 100) / newJobs.iCount),2),'%') FROM   emp_rh.job, (SELECT COUNT(job_shift_type) AS iCount FROM emp_rh.job) newJobs GROUP BY job_shift_type");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		...
		rs = stmt.executeQuery("SELECT  job_contract_type, COUNT(job_contract_type), concat(FORMAT(((COUNT(job_contract_type) * 100) / newJobs.iCount),2),'%') FROM   emp_rh.job, (SELECT COUNT(job_contract_type) AS iCount FROM emp_rh.job) newJobs GROUP BY job_contract_type");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 

		System.out.println(" - travel_to_work"); 
		rs = stmt.executeQuery("SELECT  travel_to_work, COUNT(travel_to_work), concat(FORMAT(((COUNT(travel_to_work) * 100) / newJobs.iCount),2),'%') FROM   emp_rh.job, (SELECT COUNT(travel_to_work) AS iCount FROM emp_rh.job) newJobs GROUP BY travel_to_work");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		...
		/************** Job Seeker ***************************/
		System.out.println("\n\n *** Job Seeker *** "); 
		rs = stmt.executeQuery("select count(*) FROM emp_rh.candidate");
		for (;rs.next();)
			System.out.println("Total number of jobseekers: " + rs.getString(1)); 

		System.out.println(" - Languages"); 
		rs = stmt.executeQuery("SELECT  language, COUNT(language) AS `Count`, concat(FORMAT(((COUNT(language) * 100) / newJobs.iCount),2),'%') AS `Percentage` FROM   emp_rh.candidate, (SELECT COUNT(language) AS iCount FROM emp_rh.candidate) newJobs GROUP BY language");
		for (;rs.next();) 
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getInt(2) + " candidates - " + rs.getString(3)); 

		System.out.println(" - driving_licence"); 
		rs = stmt.executeQuery("SELECT  driving_licence, COUNT(driving_licence), concat(FORMAT(((COUNT(driving_licence) * 100) / newJobs.iCount),2),'%') FROM   emp_rh.candidate, (SELECT COUNT(driving_licence) AS iCount FROM emp_rh.candidate) newJobs GROUP BY driving_licence");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 

		System.out.println(" - marital_status"); 
		rs = stmt.executeQuery("SELECT  marital_status, COUNT(marital_status), concat(FORMAT(((COUNT(marital_status) * 100) / newJobs.iCount),2),'%') FROM   emp_rh.candidate, (SELECT COUNT(marital_status) AS iCount FROM emp_rh.candidate) newJobs GROUP BY marital_status");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 

		System.out.println(" - nationality"); 
		rs = stmt.executeQuery("SELECT  nationality, COUNT(nationality), concat(FORMAT(((COUNT(nationality) * 100) / newJobs.iCount),2),'%') FROM   emp_rh.candidate, (SELECT COUNT(nationality) AS iCount FROM emp_rh.candidate) newJobs GROUP BY nationality");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 

		System.out.println(" - gender"); 
		rs = stmt.executeQuery("SELECT  gender, COUNT(gender), concat(FORMAT(((COUNT(gender) * 100) / newJobs.iCount),2),'%') FROM   emp_rh.candidate, (SELECT COUNT(gender) AS iCount FROM emp_rh.candidate) newJobs GROUP BY gender");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 
