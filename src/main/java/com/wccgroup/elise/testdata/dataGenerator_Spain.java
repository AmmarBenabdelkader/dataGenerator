 ...
 */
public class dataGenerator_Spain
{
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException, IOException
	{
		...
		while (true)
		{
			...
			System.out.println("\t- 1- Generate test data for Spain (based on ESCO)");
			System.out.println("\t- 2- Generate Lists for Spain (???)");
		...
		{
		case "1":
			generateData(0, 10); // generate test data for job and jobseeker based on ESCO occupations
			break;
		case "2":
			generateLists("es");
			break;
...
*/		
	...
	public static void assignNames()
	{
		...
		try
		{
			...
			int nbre = 0;
			query = "SELECT candidate_id, language, gender FROM emp_spain.candidate";
			...
			while (rs.next())
			{
				if (rs.getString("gender").equalsIgnoreCase("M"))
					...
					stmt2.executeUpdate(
						"Update emp_spain.Candidate set first_name=\""
							+ Names[nbre][0]
							//+ "\", middle_name= \""
	...
	public static void generateEducation()
	{
		...
		try
		{
			...
			query = "select candidate_id, birth_date, timestampdiff(year,birth_date, curdate()) age FROM emp_spain.candidate where candidate_id not in (select distinct candidate_id from emp_spain.candidate_education)";
			...
			while (rs.next())
			{
				...
				for (int i = niv; i <= 10; i++)
				{
					...
					if (rs2.next())
					{
						query = "insert into emp_spain.candidate_education (candidate_id, education_name, education_level) values ("
							+ cId
							+ ", '"
	...
	public static void GenerateJobCompanies(String country, int population)
	{
		...
		try
		{
			...
			stmt2 = conn.createStatement();
			ResultSet rs = null;
			String query = "SELECT count(*) from emp_spain.employer";
			rs = stmt.executeQuery(query);
			...
			while (rs.next())
			{
				...
				query = "insert into emp_spain.employer (employer_id, company_name, industry_sector, city, country, latitude, longitude) values ("
					+ j++
					+ ", \""
					+ rs.getString("name")
					+ "\", '"
			...
			//query = "update emp_spain.employer a set industry_sector=(select sector_id from asoc.sectors b where a.industry_sector=b.name_en) where a.industry_sector in (select name_en from asoc.sectors)";
			//stmt2.executeUpdate(query);
			System.out.println(new Date());
	...
	public static void assignJobCompany()
	{
		...
		try
		{
			...
			String query = "SELECT distinct industry_sector FROM emp_spain.employer";
			//String query = "SELECT code, title_en FROM taxonomies.sector";
			rs = stmt.executeQuery(query);
			while (rs.next())
				EmpDic.put(rs.getString(1), "");
			
			query = "SELECT employer_id, industry_sector FROM emp_spain.employer order by industry_sector, employer_id";
			rs = stmt.executeQuery(query);
			//String strValue;
			while (rs.next()) {
			...
			query = "SELECT count(*) from emp_spain.employer";
			rs = stmt.executeQuery(query);
			int counts = 0;
			if (rs.next())
			{
			...
			query = "SELECT job_id, job_title, job_sector, left(job_occupation,4) occupation from emp_spain.job";
			rs = stmt.executeQuery(query);
			rn = new Random();
			String strValue = "";
			String employer="";
			while (rs.next())
			{
				...
				query = "update  emp_spain.job set job_sector='" + job_sector + "' where job_id='" + rs.getString("job_id") + "'";
				stmt2.executeUpdate(query);
				...
				query = "update  emp_spain.job set employer_id='" + employer + "' where job_id='" + rs.getString("job_id") + "'";
	...
	public static void generateJobLanguages() throws ClassNotFoundException
	{
		...
		try
		{
			...
			query = "SELECT job_id disc_title_en, proficiency_title_en from asoc.occup_languages, emp_spain.job where left(job_occupation,5)=left(occup_code,5)";
			rs = stmt.executeQuery(query);

			while (rs.next())
			{
				query = "insert into emp_spain.job_languages (job_code, language_name, language_level) values ('"
					+ rs.getString(1)
					+ "', '"
					+ rs.getString(2)
					+ "', '"
			...
			stmt2.executeUpdate("update emp_spain.job_languages set language_level='Beginner' where language_level='Basic'");
			stmt2.executeUpdate("update emp_spain.job_languages set language_level='Advanced' where language_level='Expert'");
			stmt.close();
			stmt2.close();
			conn.close();
	...
	public static void generateJobEducation_2() throws ClassNotFoundException
	{
		...
		try
		{
			...
			while (rs.next())
			{
				query = "insert into emp_spain.job_education (job_id, education_level, education_name) values ('"
					+ rs.getString(1)
					+ "', '"
					+ rs.getString(2)
					+ "', '"
			...
			{
				stmt2.executeUpdate("update emp_spain.job_education set education_level='" + edu[i][1] + "' where education_level='" + edu[i][0] + "'");
			}
			rs.close();

			stmt2.executeUpdate("update emp_spain.job_education set edu_discipline=(select distinct disc_title_en from asoc.discipline where occup_code=job_id limit 1)");
			stmt2.executeUpdate("update emp_spain.job_education set edu_field=(select distinct field_title_en from asoc.discipline where occup_code=job_id limit 1)");
			stmt.close();
			stmt2.close();
			conn.close();
	...
	public static void generateChildren()
	{
		...
		try
		{
			...
			query = "SELECT candidate_id, birth_date, timestampdiff(year,birth_date, curdate()) age, year(birth_date) Byear FROM emp_spain.candidate where marital_status!='Single' and candidate_id not in (select distinct candidate_id from emp_spain.candidate_children)";
			//System.out.println("\t- " + query);
			rs = stmt.executeQuery(query);
			Random rn = new Random();
			...
			while (rs.next())
			{
				...
				for (int i = 1; i <= nbre; i++)
				{
					...
					query = "insert into emp_spain.candidate_children (candidate_id, child_dob, child_gender) values ("
						+ cId
						+ ", '"
						+ Byear
						+ "-"
	...
	public static void generateAddresses(String addressType, String country, Integer population)
	{
		...
		try
		{
			...
			query = "SELECT candidate_id from emp_spain.candidate where candidate_id not in (select distinct candidate_id from emp_spain.candidate_locations where address_type='" + addressType + "')";
			//query = "SELECT candidate_id from emp_spain.candidate where zipcode is null and geo_latitude is null";
			//System.out.println(query);
			rs = stmt.executeQuery(query);
			for (; rs.next();)
			{
				...
				query = "insert into emp_spain.candidate_locations (candidate_id, city, country, geo_latitude, geo_longitude, address_type, distance) values ('"
					+ rs.getInt(1)
					+ "',\""
					+ cities[nbre][0]
					+ "\", '"
	...
	public static void generateZipCodes_old(Map<String, Integer> cities)
	{
		...
		try
		{
			...
			query = "SELECT candidate_id from emp_spain.candidate where zipcode is null";
			//query = "SELECT candidate_id from emp_spain.candidate where zipcode is null and geo_latitude is null";
			//System.out.println(query);
			rs = stmt.executeQuery(query);
			Random rn = new Random();
			...
			for (; rs.next();)
			{
				...
				query = "update emp_spain.candidate set zipcode ='"
					+ addresses[0]
					+ "', city='"
					+ addresses[1]
					+ "' where candidate_id="
	...
	public static void generateCandidates(int from, int to, int nbre, String genType, String specificOccupations) throws SQLException
	{
		...
		try
		{
			...
			rs = stmt.executeQuery("select max(candidate_id) from emp_spain.candidate");
			if (rs.next())
			{
				start = rs.getInt(1) + 1;
			}
			rs = stmt.executeQuery("select count(*) from emp_spain.employer");
			if (rs.next())
			{
			...
			for (i = start; i < nbre + start; i++)
			{
				...
				query = "insert into emp_spain.candidate (candidate_id,birth_date, gender, driving_licence, marital_status, candidate_Status, nationality, work_permit_validity, language) values ("
					+ i
					+ ",'"
					+ Bdate
					+ "', '"
				...
				//stmt.executeUpdate("update emp_spain.candidate set work_permit_validity='2099-12-31'  where nationality='SA'");
...
/*				String disability = pickValue(disabilities, rn.nextInt(totals[5]));
				...
				{
					query = "insert into emp_spain.candidate_disabilities (candidate_id, disability_name) values ("
						+ i
						+ ", \""
						+ disability
						+ "\");";
...
*/
				...
				query = "insert into emp_spain.candidate_ambitions (candidate_id,job_title, occupation, availability_date, salary, salary_period, "
				...
				query = "SELECT ConceptPT, ConceptPT_en FROM escoskos.occupation_es where Parent_ISCOcode='"
					+ occupation.substring(occupation.indexOf("##") + 2) + "'";
				...
				for (j = y + age; j < year; j = j + jump)
				{
					...
					query = "insert into emp_spain.work_experience (candidate_id, job_title_we, isco_code, full_part_time, start_we, end_we, employer_id, language) values ("
						+ i
						+ ", \""
						+ jobTitle
						+ "\", \""
					...
					for (; rs.next();)
					{
						...
						//query = "insert into emp_spain.candidate_skills (job_id, skill_id) values (" + i + ", '" +  skillId + "');";
						//System.out.println("\t" + query);
						//stmt.executeUpdate(query);
					}
					rs.close();
				...
				while (it.hasNext())
				{
					...
					query = "insert into emp_spain.candidate_skills (candidate_id, skill_id, skill_name) values ("
						+ i
						+ ", '"
						+ entry.getKey()
						+ "', \""
	...
	public static void GenCandidateSkills()
	{
		...
		try
		{
			...
			String query = "SELECT candidate_id, job_title, occupation from emp_spain.candidate_ambitions where candidate_id not in (select distinct candidate_id from emp_spain.candidate_skills)";
			//System.out.println(query);
			...
			while (rsC.next())
			{
				...
				{
					Map.Entry<String, String> entry = it.next();
					query = "insert into emp_spain.candidate_skills (candidate_id, skill_id, skill_name, skill_type) values ("
	...
	public static void GenCandidateCertificates()
	{
		...
		try
		{
			...
			String query = "SELECT candidate_id, job_title, occupation from emp_spain.candidate_ambitions where candidate_id not in (select distinct candidate_id from emp_spain.candidate_Certificates)";
			//System.out.println(query);
			rsC = stmtC.executeQuery(query);
			Map<String, String> skills = new HashMap<String, String>();
			while (rsC.next())
			{
				...
				{
					Map.Entry<String, String> entry = it.next();
					query = "insert into emp_spain.candidate_certificates (candidate_id, certificate_id, certificate_type, validity_date) values ("
	...
	public static void GenJobSkills()
	{
		...
		try
		{
			...
			String query = "SELECT job_id, job_code FROM emp_spain.job where job_code not in (select distinct job_id from emp_spain.job_skills)";
			//System.out.println(query);
			rsC = stmtC.executeQuery(query);
			Map<String, String> skills = new HashMap<String, String>();
			while (rsC.next())
			{
				...
				{
					Map.Entry<String, String> entry = it.next();
					query = "insert into emp_spain.job_skills (job_id, skill_id, skill_type) values ("
	...
	public static void GenJobCertificates()
	{
		...
		try
		{
			...
			String query = "SELECT job_id, job_code FROM emp_spain.job where job_code not in (select distinct job_id from emp_spain.job_certificates)";
			//System.out.println(query);
			rsC = stmtC.executeQuery(query);
			Map<String, String> skills = new HashMap<String, String>();
			while (rsC.next())
			{
				...
				{
					Map.Entry<String, String> entry = it.next();
					query = "insert into emp_spain.job_certificates (job_id, certificate_id, certificate_type, validity_date) values ("
						+ rsC.getString(1) + ", '" + entry.getKey() + "', 'certificate', current_date());";
					stmt.executeUpdate(query);
					c++;
	...
	public static void GenCandidateSkills2()
	{
		...
		try
		{
			...
			String query = "SELECT candidate_id, job_title from emp_spain.candidate_ambitions where candidate_id not in (select distinct candidate_id from emp_spain.candidate_skills)";
			//System.out.println(query);
			rsC = stmtC.executeQuery(query);
			Map<String, String> skills = new HashMap<String, String>();
			while (rsC.next())
	...
	public static void generateCandidateLanguages()
	{
		...
		try
		{
			...
			rs = stmt.executeQuery(
				"SELECT Candidate_id, nationality FROM emp_spain.candidate where candidate_id not in (select distinct candidate_id from emp_spain.candidate_languages)");
			...
			while (rs.next())
			{
				...
				{
					Map.Entry<String, String> entry = it.next();
					query = "insert into emp_spain.candidate_languages (candidate_id, language_id, level) values ("
						+ cId
						+ ", '"
						+ entry.getKey()
						+ "', '"
	...
	public static void generateJobLanguages2()
	{
		...
		try
		{
			...
			// Generate Candidate Languages
			rs = stmt.executeQuery(
				"SELECT job_id FROM emp_spain.job where job_id not in (select distinct job_id from emp_spain.job_languages)");
			...
			while (rs.next())
			{
				...
				{
					Map.Entry<String, String> entry = it.next();
					query = "insert into emp_spain.job_languages (job_id, language_id, level) values ("
						+ cId
						+ ", '"
						+ entry.getKey()
						+ "', '"
	...
	public static void loadJobs(String trg_occup, String occupSRC)
	{
		...
		try
		{
			...
			//String query2 = "SELECT distinct "+ jobCode + ", job_title_en, job_title_es FROM " + occupSRC + occup;
			String query = "SELECT SUBSTRING_INDEX(conceptURI, '/', -1) jobCode, Parent_ISCOcode occupCode from escoskos.occupation_es where " + occup + " Parent_ISCOcode in (select left(occup_code,4) from asoc3.occupation where occup_title_es is not null)";
			String query2 = "SELECT SUBSTRING_INDEX(conceptURI, '/', -1) jobCode, ConceptPT_en job_title_en, ConceptPT  job_title_es from escoskos.occupation_es where " + occup + " Parent_ISCOcode in (select left(occup_code,4) from asoc3.occupation where occup_title_es is not null)";
			if (String.valueOf(trg_occup).length() ==99) // use 99 for work_experience
			{
				query = "SELECT distinct job_code, job_occupation from emp_spain.job where job_id>=" + from_ + " and job_id<=" + to_;
				query2 = "SELECT distinct " + jobCode + ", job_title_en, job_title_es FROM " + occupSRC 
					+ " where job_code in (SELECT distinct job_code from emp_spain.job where job_id>=" + from_ + " and job_id<=" + to_ + ")";
			}
	...
	*/
	public static void generateJobs(String trg_occup, int counts, String tableName)
	{
		...
		try
		{
			...
			rs = stmt.executeQuery("select max(job_id) from emp_spain.job");
			int start = 1;
			if (rs.next())
			{
				start = rs.getInt(1) + 1;
...
*/
				...
				query = "insert into emp_spain.job (job_id, job_code, job_title, full_part_time, job_schedule, job_occupation, salary, salary_period, "
				...
				while(rsS.next() && j++<(r+2)){
				    ...
				    query = "insert into emp_spain.job_skills (job_id, skill_id) values (" + i + ", '" +skillId + "');";
				    System.out.println("\t" + query);
...
*/
	...
	{ //synonyms and similars from Janzz
		...
		try
		{
			...
			stmt3.executeUpdate("delete from emp_spain.candidate_education");

			rs = stmt.executeQuery(
				"SELECT candidate_id, job_title, occupation, sector, 'en' language FROM emp_spain.candidate_ambitions");
			...
			while (rs.next())
			{
				...
				update = "insert into emp_spain.candidate_education (candidate_id, education_level, education_area, education_field) values (\"" + rs.getString(1) + "\",\"";
			...
			//stmt4.executeUpdate("update emp_spain.candidate_education set education_area=null where education_area=''");
			//stmt4.executeUpdate("update emp_spain.candidate_education set education_field=null where education_field=''");
	...
	{ //synonyms and similars from Janzz
		...
		try
		{
			...
			stmt4.executeUpdate("delete from emp_spain.job_education");

			rs = stmt.executeQuery(
				"SELECT job_id, job_title, job_occupation occupation, job_sector, language FROM emp_spain.job");
			...
			{
				//System.out.println(rs.getString(1) + ": " + rs.getString(2));
				update = "insert into emp_spain.job_education (job_id, education_level, education_area, education_field) values (\"" + rs.getString(1) + "\",\"";
				...
				{
					idx = 0;
					update = "insert into emp_spain.job_education (job_id, education_name, education_level, education_area, education_field) values (\"" 
...
*/					
			...
			stmt4.executeUpdate("update emp_spain.job_education set education_area=null where education_area=''");
			stmt4.executeUpdate("update emp_spain.job_education set education_field=null where education_field=''");
	...
	{
		String[] queries = {
			"delete FROM emp_spain.job_education",
			"delete FROM emp_spain.job_languages",
			"delete FROM emp_spain.work_experience",
			//"delete FROM emp_spain.employer",
			"delete FROM emp_spain.job_education",
			"delete FROM emp_spain.job_certificates",
			"delete FROM emp_spain.job_skills",
			"delete FROM emp_spain.job",
			"delete FROM emp_spain.candidate_education",
			"delete FROM emp_spain.candidate_certificates",
			"Delete from emp_spain.candidate_skills",
			"Delete from emp_spain.candidate_languages",
			"Delete from emp_spain.candidate_related",
			"Delete from emp_spain.candidate_education",
			"Delete from emp_spain.candidate_children",
			"Delete from emp_spain.candidate_disabilities",
			"Delete from emp_spain.candidate_ambitions",
			"Delete from emp_spain.candidate_locations",
			"Delete from emp_spain.candidate_benefits",
			"Delete from emp_spain.candidate_disabilities",
			"delete FROM emp_spain.candidate",
			};
	...
	public static void fullStatistics() throws ClassNotFoundException, SQLException
	{
		...
		rs = stmt.executeQuery("select count(*) FROM emp_spain.job");
		for (;rs.next();)
			System.out.println("Total number of jobs: " + rs.getString(1)); 
		
		System.out.println(" - Languages"); 
		rs = stmt.executeQuery("SELECT  language, COUNT(language) AS `Count`, concat(FORMAT(((COUNT(language) * 100) / newJobs.iCount),2),'%') AS `Percentage` FROM   emp_spain.job, (SELECT COUNT(language) AS iCount FROM emp_spain.job) newJobs GROUP BY language");
		for (;rs.next();) 
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getInt(2) + " jobs - " + rs.getString(3)); 
		
		System.out.println(" - telework"); 
		rs = stmt.executeQuery("SELECT  telework, COUNT(telework), concat(FORMAT(((COUNT(telework) * 100) / newJobs.iCount),2),'%') FROM   emp_spain.job, (SELECT COUNT(telework) AS iCount FROM emp_spain.job) newJobs GROUP BY telework");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		
		System.out.println(" - full_part_time"); 
		rs = stmt.executeQuery("SELECT  full_part_time, COUNT(full_part_time), concat(FORMAT(((COUNT(full_part_time) * 100) / newJobs.iCount),2),'%') FROM   emp_spain.job, (SELECT COUNT(full_part_time) AS iCount FROM emp_spain.job) newJobs GROUP BY full_part_time");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		
		System.out.println(" - job_shift_type"); 
		rs = stmt.executeQuery("SELECT  job_shift_type, COUNT(job_shift_type), concat(FORMAT(((COUNT(job_shift_type) * 100) / newJobs.iCount),2),'%') FROM   emp_spain.job, (SELECT COUNT(job_shift_type) AS iCount FROM emp_spain.job) newJobs GROUP BY job_shift_type");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		...
		rs = stmt.executeQuery("SELECT  job_contract_type, COUNT(job_contract_type), concat(FORMAT(((COUNT(job_contract_type) * 100) / newJobs.iCount),2),'%') FROM   emp_spain.job, (SELECT COUNT(job_contract_type) AS iCount FROM emp_spain.job) newJobs GROUP BY job_contract_type");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		
		System.out.println(" - travel_to_work"); 
		rs = stmt.executeQuery("SELECT  travel_to_work, COUNT(travel_to_work), concat(FORMAT(((COUNT(travel_to_work) * 100) / newJobs.iCount),2),'%') FROM   emp_spain.job, (SELECT COUNT(travel_to_work) AS iCount FROM emp_spain.job) newJobs GROUP BY travel_to_work");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		
		System.out.println(" - desired_gender"); 
		rs = stmt.executeQuery("SELECT  desired_gender, COUNT(desired_gender), concat(FORMAT(((COUNT(desired_gender) * 100) / newJobs.iCount),2),'%') FROM   emp_spain.job, (SELECT COUNT(desired_gender) AS iCount FROM emp_spain.job) newJobs GROUP BY desired_gender");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " jobs - " + rs.getString(3)); 
		
		/************** Job Seeker ***************************/
		System.out.println("\n\n *** Job Seeker *** "); 
		rs = stmt.executeQuery("select count(*) FROM emp_spain.candidate");
		for (;rs.next();)
			System.out.println("Total number of jobseekers: " + rs.getString(1)); 
		
		System.out.println(" - Languages"); 
		rs = stmt.executeQuery("SELECT  language, COUNT(language) AS `Count`, concat(FORMAT(((COUNT(language) * 100) / newJobs.iCount),2),'%') AS `Percentage` FROM   emp_spain.candidate, (SELECT COUNT(language) AS iCount FROM emp_spain.candidate) newJobs GROUP BY language");
		for (;rs.next();) 
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getInt(2) + " candidates - " + rs.getString(3)); 
		
		System.out.println(" - driving_licence"); 
		rs = stmt.executeQuery("SELECT  driving_licence, COUNT(driving_licence), concat(FORMAT(((COUNT(driving_licence) * 100) / newJobs.iCount),2),'%') FROM   emp_spain.candidate, (SELECT COUNT(driving_licence) AS iCount FROM emp_spain.candidate) newJobs GROUP BY driving_licence");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 
		
		System.out.println(" - marital_status"); 
		rs = stmt.executeQuery("SELECT  marital_status, COUNT(marital_status), concat(FORMAT(((COUNT(marital_status) * 100) / newJobs.iCount),2),'%') FROM   emp_spain.candidate, (SELECT COUNT(marital_status) AS iCount FROM emp_spain.candidate) newJobs GROUP BY marital_status");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 
		
		System.out.println(" - nationality"); 
		rs = stmt.executeQuery("SELECT  nationality, COUNT(nationality), concat(FORMAT(((COUNT(nationality) * 100) / newJobs.iCount),2),'%') FROM   emp_spain.candidate, (SELECT COUNT(nationality) AS iCount FROM emp_spain.candidate) newJobs GROUP BY nationality");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 
		
		System.out.println(" - gender"); 
		rs = stmt.executeQuery("SELECT  gender, COUNT(gender), concat(FORMAT(((COUNT(gender) * 100) / newJobs.iCount),2),'%') FROM   emp_spain.candidate, (SELECT COUNT(gender) AS iCount FROM emp_spain.candidate) newJobs GROUP BY gender");
		for (;rs.next();)
			System.out.println("\t- " + rs.getString(1) + ": " + rs.getString(2) + " candidates - " + rs.getString(3)); 
	...
	{
		String[] queries = {
			"delete FROM emp_spain.job where job_id>=" + from + " and job_id<=" + to,
			"delete FROM emp_spain.work_experience where candidate_id>=" + from + " and candidate_id<=" + to,
/*			"delete FROM emp_spain.employer where employer_id not in (select employer_id from emp_spain.job where job_id>="
				+ from
				+ " and job_id<="
				+ to
				+ ")",
*/			"delete FROM emp_spain.job_skills where job_id>=" + from + " and job_id<=" + to,
			"delete FROM emp_spain.candidate_certificates where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_spain.candidate_skills where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_spain.candidate_languages where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_spain.candidate_related where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_spain.candidate_education where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_spain.candidate_children where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_spain.candidate_disabilities where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_spain.candidate_ambitions where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_spain.candidate_locations where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_spain.candidate_benefits where candidate_id>=" + from + " and candidate_id<=" + to,
			"Delete from emp_spain.candidate_disabilities where candidate_id>=" + from + " and candidate_id<=" + to,
			"delete FROM emp_spain.candidate where candidate_id>=" + from + " and candidate_id<=" + to };
	...
	{
		String[] queries = {
			"delete FROM emp_spain.job where job_id>=" + from,
			"delete FROM emp_spain.work_experience where candidate_id>=" + from,
			//"delete FROM emp_spain.employer",
			"delete FROM emp_spain.job_skills where job_id>=" + from,
			"delete FROM emp_spain.candidate_certificates where candidate_id>=" + from,
			"Delete from emp_spain.candidate_skills where candidate_id>=" + from,
			"Delete from emp_spain.candidate_languages where candidate_id>=" + from,
			"Delete from emp_spain.candidate_related where candidate_id>=" + from,
			"Delete from emp_spain.candidate_education where candidate_id>=" + from,
			"Delete from emp_spain.candidate_children where candidate_id>=" + from,
			"Delete from emp_spain.candidate_disabilities where candidate_id>=" + from,
			"Delete from emp_spain.candidate_ambitions where candidate_id>=" + from,
			"Delete from emp_spain.candidate_locations where candidate_id>=" + from,
			"Delete from emp_spain.candidate_benefits where candidate_id>=" + from,
			"Delete from emp_spain.candidate_disabilities where candidate_id>=" + from,
			"delete FROM emp_spain.candidate where candidate_id>=" + from };
	...
	public static void readProperties()
	{
		...
		try
		{
			...
			input = new FileInputStream("dataGenerator_spain.properties");
