 ...
 */
public class dataGenerator_hrdf
{
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException, IOException
	{
		...
		while (true)
		{
			...
			System.out.println("\t- 1- Generate main data for HRDF (New Asoc)");
			System.out.println("\t- 2- Full Clean");
			System.out.println("\t- 8- generate additional data");
	 ...
	 */
	public static void fullClean() throws ClassNotFoundException, SQLException
	{
		String[] queries = {
	...
	public static void generateDataAsocNew(int jobCounts, int canCounts) throws SQLException, ClassNotFoundException
	{
		...
		int[][] occupations = { { 832201, sCounts } ,  { 832202, sCounts } , { 832203, sCounts }, { 832204, sCounts }, { 541401, sCounts } , { 541402, sCounts } };  
		jobCode = "job_code";
		occupCode= "occup_code";
		occupSRC = "asoc_hrdf.occupation";
		...
		generateCandidates(15, 64, to_, "specific", " and (job_code like '8322%' or job_code like '5414%'");
		
		//Generate generic candidates (usually thousands) // based on unemployed people
		generateCandidates(15, 29, (int)(canCounts*34/100), "generic", null);
		generateCandidates(30, 34, (int)(canCounts*12/100), "generic", null);
		generateCandidates(35, 39, (int)(canCounts*14/100), "generic", null);
		generateCandidates(40, 44, (int)(canCounts*12/100), "generic", null);
		generateCandidates(45, 49, (int)(canCounts*9/100), "generic", null);
		generateCandidates(50, 54, (int)(canCounts*6/100), "generic", null);
		generateCandidates(55, 59, (int)(canCounts*4/100), "generic", null);
		generateCandidates(60, 64, (int)(canCounts*2/100), "generic", null);
		
		generateCandidateLanguages();
		generateChildren();

		AssignNames_ar();
		generateAddresses("Home", "SA", 3000);
		generateAddresses("Work", "SA", 3000);
	...
	public static void GenerateJobCompanies_ar()
	{
		...
		try
		{
			...
			//System.out.println (new Date());
			query = "SELECT * FROM taxonomies_hrdf.geoname where country='SA' and feature_class='P' and population>3000";
			rs = stmt.executeQuery(query);
			int i = 0;
			while (rs.next())
			{
			...
			query = "SELECT * from taxonomies_hrdf.companies";
			rs = stmt.executeQuery(query);
			Random rn = new Random();
			int nbre = 0;
			int j = 1;
	...
	public static void assignJobCompany_ar()
	{
		...
		try
		{
			...
			String query = "update emp_hrdf.job a set job_sector = (select sector from taxonomies_hrdf.occupation_sector_ar b where left(a.job_occupation,6)=b.occup_code) where language='ar'";
			stmt2.executeUpdate(query);
			...
			query = "update emp_hrdf.job a set job_sector = (select sector_ar from taxonomies_hrdf.occupation_sector_ar b where left(a.job_occupation,6)=b.occup_code) where language='ar'";
			stmt2.executeUpdate(query);
	...
	public static void generateAddresses(String addressType, String country, Integer population)
	{
		...
		try
		{
			...
			ResultSet rs = null;
			String[][] cities = new String[200][4];
			String query = "SELECT * FROM taxonomies_hrdf.geoname where country='" + country + "'and feature_class='P' and population>" + population;
			rs = stmt.executeQuery(query);
			int i = 0;
			while (rs.next())
			{
	...
	public static void generateCandidates(int from, int to, int nbre, String genType, String condition) throws SQLException
	{
		int jump = 3;
		...
		try
		{
			...
			if (genType.equalsIgnoreCase("specific"))
				loadJobs(to_, occupSRC, condition);
			else
				loadJobs(0, occupSRC, null);
...
*/					// ********** end list of similar occupations
	...
	public static void GenCandidateSkills()
	{
		...
		try
		{
			...
			String query = "SELECT candidate_id, job_title, left(occupation,6) occupation from emp_hrdf.candidate_ambitions where candidate_id not in (select distinct candidate_id from emp_hrdf.candidate_skills)";
			//System.out.println(query);
	...
	public static void GenJobSkills()
	{
		...
		try
		{
			...
			String query = "SELECT job_id, job_title, left(job_code,6) occupation from emp_hrdf.job where job_id not in (select distinct job_id from emp_hrdf.job_skills)";
			//System.out.println(query);
			rsC = stmtC.executeQuery(query);
			Map<String, String> skills = new HashMap<String, String>();
			while (rsC.next())
			{
				...
				{
					Map.Entry<String, String> entry = it.next();
					query = "insert into emp_hrdf.job_skills (job_id, skill_id, skill_name) values ("
						+ rsC.getString(1)
						+ ", '"
						+ entry.getKey()
						+ "', \""
	...
	public static void generateJobLanguages2()
	{
		...
		try
		{
			...
			// Generate Candidate Languages
			rs = stmt.executeQuery(
				"SELECT job_id FROM emp_hrdf.job where job_id not in (select distinct job_id from emp_hrdf.job_languages)");
			...
			while (rs.next())
			{
				...
				{
					Map.Entry<String, String> entry = it.next();
					query = "insert into emp_hrdf.job_languages (job_id, language_id, level) values ("
						+ cId
						+ ", '"
						+ entry.getKey()
						+ "', '"
	 ...
	 * @return void
	 */
	public static void loadJobs(int trg_occup, String occupSRC, String condition)
	{
		Connection conn = null;
		Statement stmt = null;
		try
		{
			...
			conn = DriverManager.getConnection(DB_URL, USER, PASS);
			stmt = conn.createStatement();
			String occup=" where length(" + jobCode + ") >=4";
			ResultSet rs;
			if (String.valueOf(trg_occup).length() > 5)
				occup = " where " + jobCode + " like '" + trg_occup + "%'"  + (condition!=null?condition:"");

			if (trg_occup==0)
				occup = " where left(" + jobCode + ",1) not in " + exclude + (condition!=null?condition:"");

			String query = "SELECT distinct "+ jobCode + ", left(" + jobCode + ",4) " + occupCode + " FROM " + occupSRC + occup;
	...
	*/
	public static void generateJobs(int trg_occup, int counts, String tableName)
	{
		...
		try
		{
			...
			loadJobs(trg_occup, tableName, null);
...
*/
	...
	public static void readProperties()
	{
		...
		try
		{
			...
			input = new FileInputStream("dataGenerator_hrdf.properties");
	...
	public static void readBatchQueries(String type)
	{
		...
		try
		{
			...
			input = new FileInputStream("batchQueries_hrdf.properties");
			prop.load(input);
			for (int i=0; i<10; i++)
